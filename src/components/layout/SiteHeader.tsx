        ...
        <div className="hidden items-center gap-6 lg:flex">
          {NAVIGATION.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-sm font-semibold text-slate-600 transition hover:text-[#009EDB]"
            >
              {item.label}
            </a>
          ))}
          ...
          <nav className="flex flex-col gap-1">
            {NAVIGATION.map((item) => (
              <a
                ...
                className="rounded-xl px-4 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-50 hover:text-[#009EDB]"
              >
                {item.label}
              </a>
            ))}
