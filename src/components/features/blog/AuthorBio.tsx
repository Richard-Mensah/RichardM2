...
export default function AuthorBio({ author }: { author: Author }) {
  return (
    <aside className="mt-14 rounded-3xl border border-slate-200 bg-slate-50 p-6 md:p-8">
      <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#0077FF]">About the author</p>
      <div className="mt-4 flex flex-col gap-5 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0">
          ...
          {author.role && <p className="mt-0.5 text-sm font-semibold text-[#0077FF]">{author.role}</p>}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {author.linkedin && (
              <a
                ...
                className="rounded-full border border-slate-300 px-4 py-1.5 text-xs font-bold text-slate-700 transition hover:border-[#0077FF] hover:text-[#0077FF]"
              >
                LinkedIn
              </a>
            )}
