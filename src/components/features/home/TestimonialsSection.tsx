...
function Stars({ accentClass = "text-[#4f8bff]" }: { accentClass?: string }) {
  return (
    ...
    <div className="px-5 py-16 md:px-8 md:py-24">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.32em] text-[#7fb0ff]">
              Testimonials
            </p>
            <h2 className="mt-4 text-balance text-3xl font-black tracking-[-0.03em] text-white md:text-4xl">
              In their own words.
            </h2>
          ...
          <div className="flex shrink-0 gap-2">
            <button
              ...
              className="grid h-11 w-11 place-items-center rounded-full border border-white/20 text-white transition hover:border-[#4f8bff] hover:text-[#7fb0ff] disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:border-white/20 disabled:hover:text-white"
            >
              <ChevronLeft size={20} />
            </button>
            <button
        ...
        <div className="mt-10 overflow-hidden">
          ...
          >
            {testimonials.map((t, i) => (
              ...
              >
                <article className="glass flex h-full min-h-[300px] flex-col rounded-2xl p-6">
                  <span
                  ...
                  <p className="mt-2 flex-1 text-sm leading-[1.8] text-[#a9bcdc] [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:4] overflow-hidden">
                    {t.quote}
                  </p>
                  <button
                    type="button"
                  ...
                  <div className="mt-5 flex items-center gap-3 border-t border-white/10 pt-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate text-xs text-[#7e92b6]">{t.role}</p>
                    </div>
          ...
          {Array.from({ length: maxIndex + 1 }).map((_, i) => (
            <button
              ...
              className={cn(
                ...
                i === index ? "w-8 bg-[#4f8bff]" : "w-2 bg-white/20 hover:bg-white/40"
              )}
            />
          ))}
        </div>
        ...
        >
          ...
          >
            <button
              ...
              onClick={close}
              aria-label="Close"
              className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full text-[#9fb3d1] transition hover:bg-white/10 hover:text-white"
            >
              <X size={18} />
            </button>
            ...
            <p className="mt-2 text-base leading-[1.9] text-[#cdd9ee]">{active.quote}</p>

            <div className="mt-6 flex items-center gap-3 border-t border-white/10 pt-5">
              <Avatar initials={active.initials} accent={active.accent} />
              <div className="min-w-0">
                <p className="text-sm font-black text-white">{active.name}</p>
                <p className="mt-0.5 text-xs text-[#7e92b6]">{active.role}</p>
              </div>
              <div className="ml-auto">
                <Stars />
              </div>
